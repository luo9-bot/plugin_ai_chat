...
#### 5.4 心理状态 (`mental_state/`)

| 子模块 | 功能 |
|--------|------|
| `concerns.rs` | 关注点管理 (ConcernCategory) |
| `deliberations.rs` | 思虑管理 |
| `defects.rs` | 缺陷检测 (DefectType) |

#### 5.5 自我记忆 (`self_memory/`)

- `store.rs`：自我思考存储 (ThoughtCategory: Reflection/Experience/Plan/Feeling)
- `reflect.rs`：自我反思
- `sync.rs`：自我记忆同步

#### 5.6 人格系统 (`personality/`)
...
| 模块 | 功能 |
|------|------|
| `runtime.rs` | 状态管理、免打扰时段、日期提醒 |
| `trigger.rs` | 主动消息触发判断、群氛围检测 |
| `generate.rs` | 主动消息内容生成 |
| `motivation.rs` | 主动消息动机系统 |
