...
Replyer (回复生成)
  └─ 注入人格+记忆+情绪+表达习惯+正在进行的事（含真实素材）+ASI反馈
...
### 真实活动与真实世界（`world/` + `activity/`）

「她在看直播」若只是台词，就永远是假的。现在的活动是三件真实的事：

1. **由行为开合**：她用 `do_activity` 工具真的开始做/停下（表达与回神
   两条路径共用），不再从她的话里嗅关键词（旧版"休息"→ 睡 8 小时）
2. **有真实素材**：`world/` 巡视她关注的 B 站直播间——开播/下播是真实
   状态转变（入流 + 感官唤醒），直播中的弹幕/标题/在线人数递进她的活动
   素材表；她说起这件事时引用的细节是真的。看直播只能看**此刻真的在播**
   的房间，没有房间在播就诚实地说没有
3. **有代价**：活动占用注意力（`occupancy`）——打字变慢、话变短、被打断
   是常态；下播事件会提前收尾她的观看

素材与开播/下播会沉淀为经历入流（72h 淡忘），供回忆与睡前日记引用。
没在看的时候弹幕不入流——不在场就听不到，这是诚实。
...
world:
  ...
  announce_group: 0        # 开播时允许主动发言的群（0=只在心里想）
  live_rooms:
    - room_id: 21452505
      name: "七海"          # 她对主播的称呼（接口不给名字）
```
