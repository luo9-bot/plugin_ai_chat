...
### 世界模型：记得与预期（`world/model.rs`）

巡视只给她「此刻」，但一个活在世界里的存在对世界是有**积累与预期**的。
世界模型把一次次巡视沉淀成房间的持久记忆（`data/mind/world_model.json`），
再把「与平常的偏差」作为机械事实递给她——由此她不再是一个照着台词演的
角色，而是一个**住在有规律的世界里、规律被打破时看得见**的存在：

- **熟脸**：按弹幕出现次数记住这房间常见的人（`regulars`）——她说「那个
  总来的今天没来」是有依据的（缺席是真实信号，不是情绪）
- **作息**：从场次历史（`sessions`）量出「平常约几点开播、一般播多久」
- **预期与意外**：开播/下播时刻、时长、在线峰值与「平常」做**预测误差**——
  「比平常晚了约 2 小时开播」是数值比较，不是「好意外呀」；惊不惊由她诠释
- **现场质感**：看直播时随行的在线轨迹 + 弹幕节奏（每两分钟约几条）——
  她说这场「热闹/冷清」时，背后是量出来的数

铁律不变：零抒情、事实从真实历史聚合、不写她的内心（那是回神专属）。
这些事实同时进「你关注的直播间」感官块（对话/回神）与意识流（供回忆/日记）。
