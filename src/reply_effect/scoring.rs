use super::store::{FollowupMessage, ReplyEffectRecord};
...
pub(crate) fn calculate_behavior_score(record: &ReplyEffectRecord) -> f64 {
    ...
    // 是否继续对话（2+ 轮）
    let continue_2turns = if target.len() >= 2 {
        1.0
    ...
    // 用户消息长度（参与度）
    let avg_len: f64 = target
        .iter()
        .map(|f| f.content.chars().count() as f64)
        .sum::<f64>()
        / target.len().max(1) as f64;
    let user_expansion = ((avg_len - 8.0) / 42.0).clamp(0.0, 1.0);

    // 无纠正
    let no_correction = if target
        .iter()
    ...
    (0.30 * continue_2turns
        + 0.25 * next_user_sentiment
        + 0.20 * user_expansion
        + 0.15 * no_correction
        + 0.10 * no_abort)
        .clamp(0.0, 1.0)
...
fn estimate_sentiment(messages: &[&FollowupMessage]) -> f64 {
    ...
    f64::clamp(
        0.5 + 0.2 * positive_count - 0.25 * negative_count - 0.15 * repair_count,
        0.0,
        1.0,
    )
}
...
fn calculate_relational_score(record: &ReplyEffectRecord) -> f64 {
    ...
    (0.35 * social_presence + 0.25 * warmth + 0.25 * competence + 0.15 * appropriateness)
        .clamp(0.0, 1.0)
}
...
fn calculate_friction_score(record: &ReplyEffectRecord) -> f64 {
    ...
    let explicit_negative = record
        ...
        .iter()
        .map(|f| {
            let weight = if f.user_id == record.target_user {
                1.0
            } else {
                0.65
            };
            let max_match = NEGATIVE_PATTERNS
                .iter()
    ...
    let repair_loop = record
        ...
        .map(|f| {
            ...
            if REPAIR_PATTERNS.iter().any(|p| f.content.contains(p)) {
                weight
            } else {
                0.0
            }
...
pub(crate) fn judge_with_llm(record: &ReplyEffectRecord) -> Option<LlmJudgeScores> {
    ...
    // 构建 followup 消息文本
    let followup_text: Vec<String> = record
        .followups
        .iter()
        .take(5)
        .map(|f| {
