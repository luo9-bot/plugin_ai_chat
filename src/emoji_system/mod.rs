...
use luo9_sdk::Msg;
pub use store::EmojiEntry;
pub use manager::{EmojiSelection, register_emoji, register_from_cq, select_emoji_vlm, update_usage, get_emoji_path, get_stats, maintenance};
...
pub fn send_emoji(
    ...
    // 使用 VLM 选择最合适的表情包
    let selection = manager::select_emoji_vlm(context, target_emotion, recent_hashes)
...
pub fn get_emoji_context() -> String {
    let (_total, registered) = manager::get_stats();
    if registered == 0 {
        return String::new();
    }
    format!(
        "# 表情包系统\n你有 {} 个可用的表情包。当对话需要情感表达时，可以调用 send_emoji 工具发送合适的表情包。",
        registered
    )
}
