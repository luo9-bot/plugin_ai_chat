...
pub static WEAK_PATTERNS: &[WeightedPattern] = &[
    ...
    WeightedPattern {
        ...
        category: RiskCategory::Sexual,
        score: 0.30,
        suppressors: &["了解", "学习", "研究", "分析", "探讨", "思考", "调查"],
        suppress_window: 4,
    },
    WeightedPattern {
        pattern: "摩擦",
    ...
    WeightedPattern {
        ...
        category: RiskCategory::Violence,
        score: 0.30,
        suppressors: &["鸡", "酱", "弹", "肉", "鱼", "虾", "酥", "排", "薯条"],
        suppress_window: 3,
    },
    WeightedPattern {
        pattern: "毒死",
...
pub fn match_patterns(segments: &[String]) -> PatternScores {
    ...
    for seg in segments {
        ...
        combined.sexual = combined
            .sexual
            .max(strong.sexual)
            .max(weak.sexual)
            .max(combos.sexual);
        combined.violence = combined
            .violence
            .max(strong.violence)
            .max(weak.violence)
            .max(combos.violence);
        combined.illegal = combined
            .illegal
            .max(strong.illegal)
            .max(weak.illegal)
            .max(combos.illegal);
        combined.jailbreak = combined
            .jailbreak
            .max(strong.jailbreak)
            .max(weak.jailbreak)
            .max(combos.jailbreak);
        combined.emotional = combined
            .emotional
            .max(strong.emotional)
            .max(weak.emotional)
            .max(combos.emotional);
    }
    combined
}
