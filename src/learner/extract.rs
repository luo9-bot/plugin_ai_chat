...
pub fn learn_from_messages(group_id: u64, messages: &[(u64, String)]) {
    ...
    if now.saturating_sub(s.last_learned.get(&group_id).copied().unwrap_or(0)) < LEARN_INTERVAL_SECS
    {
        return;
    }
                ...
                && let Ok(parsed) = serde_json::from_str::<serde_json::Value>(&json_str)
            {
                if let Some(exprs) = parsed.get("expressions").and_then(|v| v.as_array()) {
                    for expr in exprs {
                        ...
                        if sit.is_empty() || sty.is_empty() {
                            continue;
                        }
                        if let Some(e) = s.expressions.iter_mut().find(|e| {
                            text_sim(&e.situation, sit) > SIMILARITY_THRESHOLD
                                && text_sim(&e.style, sty) > SIMILARITY_THRESHOLD
                        }) {
                            e.count += 1;
                        } else {
                            s.expressions.push(ExpressionHabit {
                                situation: sit.into(),
                                style: sty.into(),
                                count: 1,
                ...
                if let Some(jargons) = parsed.get("jargon").and_then(|v| v.as_array()) {
                    for j in jargons {
                        ...
                        let t = match j.get("type").and_then(|v| v.as_str()).unwrap_or("") {
...
fn infer_jargon_meaning(jargon: &str, context_lines: &[String]) -> String {
    // 取包含该黑话的上下文（前后各 3 条）
    let context: Vec<&str> = context_lines
        .iter()
